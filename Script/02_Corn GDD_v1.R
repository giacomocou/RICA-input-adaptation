### Script for CORN - GDD specification ########################################
...
# load GDD and KDD
temps <- readRDS("Data/Raw data/Temperature/Corn_GDD_KDD_hist.rds")
...
if(F){ # safeguard since this takes a while to run
  ...
  saveRDS(structural_model, "Output/Corn/Corn_GDD_structural_model.RDS")
}

# use already saved model
structural_model <- readRDS("Output/Corn/Corn_GDD_structural_model.RDS")
...
#write.csv(summary_table, "Output/Corn/Corn_summary_structural_GDD.csv", row.names= T, quote=FALSE)
