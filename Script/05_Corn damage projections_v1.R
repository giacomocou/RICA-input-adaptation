...
# main df
d <- readRDS("Data/Processed data/Corn_cleaned_merged.RDS")

# structural model
structural_model <- readRDS("Output/Corn/Corn_structural_model.RDS")

# read projections ssp370
projections <- readRDS("Data/Processed data/Corn_projections_unbiased_ssp370.RDS")
...
for (scen_name in scenario_columns) {
  ...
  projections_and_sd <- d_proj %>%
    mutate(
      ...
      scenario_used        = scen_name  #track which scenario generated this
    )
  
  # save dataframe
  file_name <- paste0("Output/Corn/Projections/Corn_projections_ssp370_2050_", scen_name, ".RDS")
