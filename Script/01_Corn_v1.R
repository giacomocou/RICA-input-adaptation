...
prec <- prec %>%
  mutate(sum_prec = rowSums(select(., prec3:prec9), na.rm = TRUE)) %>%
...
input_prices <- input_prices %>%
  ...
  mutate(price_fertiliser = price_fertiliser/100, # we use base 1
...
output_prices <- output_prices %>%
  rename(year = anno) %>%
  select(year, price_corn)

d <- left_join(d,output_prices, by = "year")

# compute relative prices
d <- d %>% 
  mutate(rel_price_n = price_fertiliser/price_corn,
         rel_price_ph = price_pesticide/price_corn,
...
saveRDS(d, "Data/Processed data/Corn_cleaned_merged.RDS")
...
d_cor <- d_cor %>%
  distinct(farm_code, .keep_all = TRUE) %>%   # Keep only unique farm_code values
...
p <- ggplot() +
  geom_sf(data = italy_regions, fill = "#f0f0f0", color = "lightgray") +  # Plot regional borders
  geom_sf(data = farms_sf, color = "darkgreen", aes(size = avg_area), alpha = 0.5) +  # Plot farm points
  scale_size_continuous(range = c(0.5, 3)) +
  labs(title = "Corn", size = "Average crop area (ha)") +
  theme_minimal() +
  theme(
...
p
#ggsave("Output/Corn/map_corn.tiff", p, bg = "white", width = 6, height = 8, dpi = 600, compression = "lzw")
...
descriptive_stats <- d %>%
  select(mean_temp, sum_prec, yield, area, price_corn, price_fertiliser, price_pesticide, fertiliser_per_area_deflated, pesticide_per_area_deflated) %>%
  reframe(
...
descriptive_stats$Variable <- c("Average temperature", "Total precipitation", "Yield", "Area", "Corn price (2010=100)", "Fertilizer price (2010=1)", "Pesticide price (2010=1)", "Fertilizer applications per Ha", "Pesticide applications per Ha")
...
summary(model_Y_fixest) # clusters automatically for farm_code
...
vcov_reduced_conley <- conleyreg(
  ...
  data = d,
  unit = "farm_code",
  dist_cutoff = 100,  # Distance cutoff in km
  vcov = TRUE,
  lat = "latitude",
  lon = "longitude"
)
...
if(F){ # safeguard since this takes a while to run, use only first time you run the code or if you want to re-estimate the model
  ...
  saveRDS(structural_model, "Output/Corn/Corn_structural_model.RDS")
}

# use already saved model
structural_model <- readRDS("Output/Corn/Corn_structural_model.RDS")
...
#write.csv(summary_table, "Output/Corn/Corn_summary_structural.csv", row.names= T,quote=FALSE)
...
#ggsave(filename = "Output/Corn/ME_Corn.tiff", plot, width = 8, height = 8, dpi = 600, bg = "white", compression = "lzw")
...

{
  ...
  temp_seq <- seq(15, 21, length.out = 100)
  ...
  fertilizer_demand <- ggplot(plot_data, aes(x = temp_seq, y = predicted_ln_yield)) +
    geom_line(color = "darkgreen", size = 1) +                      # Plot the response function
    geom_ribbon(aes(ymin = lower_bound, ymax = upper_bound),   # Confidence bands
  ...
  fertilizer_demand
  #ggsave("Submission_output/Corn_fertilizer_demand.png", plot = fertilizer_demand, width = 8, height = 6)
}
...

{
  ...
  pesticide_demand <- ggplot(plot_data, aes(x = temp_seq, y = predicted_ln_yield)) +
    geom_line(color = "darkred", size = 1) +                      # Plot the response function
    geom_ribbon(aes(ymin = lower_bound, ymax = upper_bound),   # Confidence bands
  ...
  pesticide_demand
  #ggsave("Submission_output/Corn_pesticide_demand.png", plot = pesticide_demand, width = 8, height = 6)
}
...

{
  ...
  #write.csv(productivity_parameters, "Submission_output/Corn_productivity_parameters.csv", row.names= F,quote=FALSE)
}
